...
// new enum to support return datatype of functions
...
/* new class to store return Datatype information. */
class NDataType : public NStatement {
public:
  const dataType dtype;
  std::string dname;
  NDataType(const dataType d_type): dtype(d_type){
	  switch(d_type){
		  case VOID: dname="void"; break;
		  case INT: dname="int";break;
		  default: dname="";
	  }
  }
  virtual void print() const{
...
public:
    ...
    PNODE(const NDataType) dType; // Store datatype of return type 
    NExternDeclaration(PNODE(const NIdentifier) id,
            const NVariableList &arguments, PNODE(const NDataType) d_type) :
            id(id), arguments(arguments), dType(d_type)  {
    }
    virtual void print() const{
      if(nullptr==this){return;}
      std::cout<<'['<<std::endl;
...
public:
    ...
    PNODE(const NDataType) dType; // Stores datatype of return type 
    NFunctionDeclaration(PNODE(const NIdentifier) id,
            const NVariableList &arguments,
            PNODE(const NBlock) block, PNODE(const NDataType) d_type) :
            id(id), arguments(arguments), block(block), dType(d_type) {
    }
    virtual void print() const{
      if(nullptr==this){return;}
      std::cout<<'['<<std::endl;

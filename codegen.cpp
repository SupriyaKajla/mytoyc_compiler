...
llvm::Value* NBlock::codeGen(CodeGenContext& context) const {
    ...
    // store the last statment in the list to check if laststatement is return statement or not.
    const PNODE(NStatement) lastStatement = nullptr;
    ...
    // Type cast to check if the last statement is return statement or not.
    const NReturnStatement *rs = dynamic_cast<const NReturnStatement*>(lastStatement);
    if (rs == nullptr)
	    logError("last statement in block cannot be empty, it should always have return statement.");
...
llvm::Value* NReturnStatement::codeGen(CodeGenContext& context) const {
    ...
    // to support null expression as return statment can be with empty expression.
...
llvm::Value* NExternDeclaration::codeGen(CodeGenContext& context) const {
    ...
    /* create funtion type on basis of return type of functions */
    llvm::FunctionType *ftype = nullptr;
    if (dType->dtype == dataType::INT) {
...
llvm::Value* NFunctionDeclaration::codeGen(CodeGenContext& context) const {
    ...
    std::cerr << "Return Type of function: " << dType->dname << std::endl;
    for (it = arguments.begin(); it != arguments.end(); it++) {
        argTypes.push_back(context.getIntType());
    }
    ...
    /* create funtion type on basis of return type of functions */
    llvm::FunctionType *ftype = nullptr;
    if (dType->dtype == dataType::VOID) {
	    ftype = llvm::FunctionType::get(
			    context.getVoidType(),
			    llvm::makeArrayRef(argTypes), false);

	    /* check each statement and verify if the return type is void, then return statement can not have any expression */ 
	    for(auto elt:this->block->statements) {
		    const NReturnStatement *rs = dynamic_cast<const NReturnStatement*>(elt);
		    if (rs != nullptr) {
			    if (rs->expression != nullptr) {
				    logError("void return type cannot have expression after return statement" );
			    }
		    }
	    }
    } else {
